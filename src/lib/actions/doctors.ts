...
export async function getDoctors(){
    try{
        //get doctors from the database and get doctors that have appointments and order them by createdAt in descending order
...
export async function getAvailableDoctors() {
    try {
        //find active doctors and the amount of appointments they have and order them by name in ascending order
    const doctors = await prisma.doctor.findMany({
        where: { isActive: true },
        include: {
        _count: {
